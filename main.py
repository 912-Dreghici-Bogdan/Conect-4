...
def score_position(board,piece):
    score = 0
    ##Score center column
    center_array = row_array = [int(i) for i in list(board[:, COLUMN_COUNT//2])]
...
def draw_board(board):
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            ...
            pygame.draw.circle(screen, BACKROUND, (
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            pygame.draw.rect(screen, BACKROUND, (0, 0, width, SQUARESIZE))
            posx = event.pos[0]
            if turn == PLAYER:
                pygame.draw.circle(screen, COLOUR1, (posx, int(SQUARESIZE / 2)), RADIUS)
        pygame.display.update()
            ...
            if turn == PLAYER:
                ...
                if is_valid_location(board, col):
                    ...
                    if winning_move(board, PLAYER_PIECE):
                        label = myfont.render("Player 1 wins!!", 1, COLOUR1)
